...
public:
    ...
    json () : mguard (JSONNULL) { mboolean = false; }
    json (json const& x) : mguard (x.mguard) { copy_data (x); }
    json (json&& x) : mguard (x.mguard) { move_data (std::move (x)); }
    ~json () { destroy (); }

    json (std::nullptr_t x) : mguard (JSONNULL) { mboolean = false; }
    json (bool x) : mguard (BOOLEAN) { mboolean = x; }
    json (int x) : mguard (INTEGER) { minteger = x; }

    json (double x) : mguard (NUMBER)
    {
    ...
    json (wchar_t const* x) : mguard (STRING) { new (&mstring) std::wstring (x); }
    json (std::wstring const& x) : mguard (STRING) { new (&mstring) std::wstring (x); }
    json (std::wstring&& x) : mguard (STRING) { new (&mstring) std::wstring (std::move(x)); }
    json (array const& x) : mguard (ARRAY) { new (&marray) array (x); }
    json (array&& x) : mguard (ARRAY) { new (&marray) array (std::move(x)); }
    json (object const& x) : mguard (OBJECT) { new (&mobject) object (x); }
    ...
    variation guard () const { return mguard; }
